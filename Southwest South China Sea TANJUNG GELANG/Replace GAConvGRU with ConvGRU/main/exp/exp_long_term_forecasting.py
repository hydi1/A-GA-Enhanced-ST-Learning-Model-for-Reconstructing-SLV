...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def get_r2_eff(self, preds, targets):
        """
        计算有效解释率 (R²) - 适用于去重叠反归一化后的测试集数据
        
        有效解释率的定义：
        R² = 1 - (SS_res / SS_tot)
        其中：
        - SS_res: 残差平方和 = Σ(y_pred - y_true)²
        - SS_tot: 总平方和 = Σ(y_true - y_mean)²
        - R² ∈ (-∞, 1]：值越接近1，模型拟合效果越好；为负表示模型性能不如均值模型
        
        Args:
            preds (array-like): 预测值，形状为 (N,) 或 (N, 1)
            targets (array-like): 真实目标值，形状为 (N,) 或 (N, 1)
        ...
        Example:
            ...
            >>> preds_full = outputs_full_unnormalized  # 去重叠反归一化的预测值
            >>> targets_full = batch_y_full_unnormalized  # 去重叠反归一化的真实值
    ...
    def test(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            all_outputs = []  # 保存所有预测结果（归一化尺度，flatten 后）
            all_batch_y = []  # 保存所有真实值（归一化尺度，flatten 后）

            # 遍历每个 batch：收集 outputs / y + 计算 batch(归一化)指标
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # 保存归一化尺度数据（flatten 后保存，用于后续去重叠拼接）
                all_outputs.append(outputs_np)  # shape [B*L, 1]
                all_batch_y.append(batch_y_np)  # shape [B*L, 1]
