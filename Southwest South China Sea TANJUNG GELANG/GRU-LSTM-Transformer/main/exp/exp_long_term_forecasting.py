...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        import pandas as pd  # ✅ 确保已导入（用于保存Excel）
        ...
        # 基于原始 batch 的损失（反归一化）
        # =========================
        rmse_batch = AverageMeter()
        mae_batch = AverageMeter()

        # =========================
        # 基于去重叠后的损失（反归一化）
        # =========================
        rmse_full = AverageMeter()
        mae_full = AverageMeter()

        # =========================
        # ✅ 新增：归一化尺度的 RMSE/MAE（batch 和 去重叠）
        # =========================
        ...
        self.model.eval()
        with torch.no_grad():
            all_outputs = []  # 保存归一化预测（flatten后）
            all_batch_y = []  # 保存归一化真实（flatten后）
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                f_dim = -1 if self.args['features'] == 'MS' else 0
                outputs = outputs[:, :, f_dim:]  # [B, L, 1]
                batch_y = batch_y[:, :, f_dim:]  # [B, L, 1]

                # =========================
                # ✅ 新增：基于 batch 的归一化 RMSE/MAE
                # =========================
                # 用 tensor 直接算（归一化尺度）
                ...
                # ⚠️ 你这里写死 reshape(-1, 6, 1)，确保 seq_len=6，否则改为 self.args['seq_len']
                ...
                print('batch_y_unnormalized', batch_y_unnormalized_tensor.shape,
                      'outputs_unnormalized', outputs_unnormalized_tensor.shape)

                rmse_batch.update(
                    rmse_loss(outputs_unnormalized_tensor, batch_y_unnormalized_tensor).item(),
                    batch_x.size(0)
                )
                mae_batch.update(
                    mae_loss(outputs_unnormalized_tensor, batch_y_unnormalized_tensor).item(),
                    batch_x.size(0)
                )

                # 保存归一化输出用于去重叠
                all_outputs.append(outputs_cpu)  # [B*L, 1] normalized
                all_batch_y.append(batch_y_cpu)  # [B*L, 1] normalized
            ...
            print("outputs_full shape:", outputs_full.shape)
            print("batch_y_full shape:", batch_y_full.shape)
            ...
            for batch_outputs, batch_y_arr in zip(all_outputs, all_batch_y):
                ...
                for i in range(batch_size):
                    ...
                    batch_y_full[start_idx:end_idx] += batch_y_arr[i * seq_len:(i + 1) * seq_len]
            ...
            rmse_full.update(rmse_loss(outputs_full_tensor, batch_y_full_tensor).item(), full_length)
            mae_full.update(mae_loss(outputs_full_tensor, batch_y_full_tensor).item(), full_length)

            # =========================
            # 保存去重叠后的反归一化序列到 Excel（你原来的格式保留）
            # =========================
            outputs_full_unnormalized = outputs_full_unnormalized.T  # (1, T)
            batch_y_full_unnormalized = batch_y_full_unnormalized.T  # (1, T)

            print("batch_y_full_unnormalized:", batch_y_full_unnormalized.flatten())
            print("outputs_full_unnormalized:", outputs_full_unnormalized.flatten())

            batch_y_columns = [f'batch_y_t{i}' for i in range(full_length)]
            outputs_columns = [f'outputs_t{i}' for i in range(full_length)]

            batch_y_df = pd.DataFrame(batch_y_full_unnormalized, columns=batch_y_columns)
            outputs_df = pd.DataFrame(outputs_full_unnormalized, columns=outputs_columns)
            ...
            # =========================
            # ✅ 新增：把归一化/反归一化的指标也写入同一个Excel（新sheet）
            # =========================
            metrics_df = pd.DataFrame([{
                "rmse_batch_norm": rmse_batch_norm.avg,
                "mae_batch_norm": mae_batch_norm.avg,
                "rmse_full_norm": rmse_full_norm.avg,
                "mae_full_norm": mae_full_norm.avg,
                "rmse_batch_denorm": rmse_batch.avg,
                "mae_batch_denorm": mae_batch.avg,
                "rmse_full_denorm": rmse_full.avg,
                "mae_full_denorm": mae_full.avg,
                "seq_len": seq_len,
                "full_length": full_length,
                "lr": self.args.get("learning_rate", None),
                "seed": self.args.get("model_id", None),
                "setting": str(setting)
            }])

            # 追加写入（如果没有该sheet则创建；如果存在则覆盖）
            with pd.ExcelWriter(save_path_excel, engine="openpyxl", mode="a", if_sheet_exists="replace") as writer:
                metrics_df.to_excel(writer, sheet_name="metrics", index=False)

        # =========================
        # 输出损失（新增归一化）
        # =========================
        print('Batch-based (normalized): rmse:{}, mae:{}'.format(rmse_batch_norm.avg, mae_batch_norm.avg))
        print(
            'Full-sequence (de-overlapped, normalized): rmse:{}, mae:{}'.format(rmse_full_norm.avg, mae_full_norm.avg))
        ...
        with open(save_path_txt, 'a') as f:
            if isinstance(setting, dict) and 'epoch' in setting:
                test_result = (
                    ...
                    f"Batch(norm): rmse: {rmse_batch_norm.avg}, mae: {mae_batch_norm.avg}, "
                    f"Full(norm): rmse: {rmse_full_norm.avg}, mae: {mae_full_norm.avg}, "
                    f"Batch(denorm): rmse: {rmse_batch.avg}, mae: {mae_batch.avg}, "
                    f"Full(denorm): rmse: {rmse_full.avg}, mae: {mae_full.avg}"
                )
            else:
                test_result = (
        ...
        return {
            ...
            'rmse_full_denorm_avg': rmse_full.avg,
            'mae_full_denorm_avg': mae_full.avg,
        }
    ...
    #
    #             # 计算 MSE 和 MAE 需要使用反归一化后的数据
    #             rmse.update(rmse_loss(outputs_unnormalized_tensor, batch_y_unnormalized_tensor).item(), batch_x.size(0))
    #             print(batch_x.size(0))
    #             mae.update(mae_loss(outputs_unnormalized_tensor, batch_y_unnormalized_tensor).item(), batch_x.size(0))
    #
    #             #
    #             # 提取每个 batch 的第一个时间步的数据
    #             first_time_step_batch_y = batch_y_cpu[:, :]  # 第一个时间步
    #             first_time_step_outputs = outputs_cpu[:, :]  # 第一个时间步
    #
    #             # 将第一个时间步的数据添加到列表中
    #             all_batch_y.append(first_time_step_batch_y)
    #             all_outputs.append(first_time_step_outputs)
    #         # 将所有 batch 的第一个时间步数据合并
    #         all_batch_y = np.concatenate(all_batch_y, axis=0)
    #         all_outputs = np.concatenate(all_outputs, axis=0)
    #         # 转换为 pandas DataFrame
    #         batch_y_df = pd.DataFrame(all_batch_y, columns=['batch_y_first_time_step'])
    #         outputs_df = pd.DataFrame(all_outputs, columns=['outputs_first_time_step'])
    #         # 拼接 batch_y 和 outputs 的第一个时间步
    #         results_df = pd.concat([batch_y_df, outputs_df], axis=1)
    #
    #         # 保存到 Excel 文件
    #         save_path = r'D:\project\SOFTS_TS\output_test\output_y17.xlsx'
    ...
    #     # 格式化每次测试的结果
    #     if isinstance(setting, dict) and 'epoch' in setting:
    #         test_result = f"Epoch: {setting['epoch']},lr={self.args['learning_rate']}, rmse: {rmse}, mae: {mae}"
    #     else:
    #         test_result = f"lr={self.args['learning_rate']},rmse: {rmse}, mae: {mae}"  # 如果 setting 不是字典，可能不包含 epoch
    #
    #     # 保存到 txt 文件中
    #
    #     save_path = os.path.join(r"D:\project\GRU\SOFTS-main\test_result", 'test_results.txt')
    #     with open(save_path, 'a') as f:  # 使用 'a' 模式是为了追加写入
    #         f.write(test_result + '\n')  # 每次测试结束后写入一行
    #
    #     return