...
class Dataset_Npy(Dataset):
    ...
    def __read_data__(self):
        ...
        data_raw = np.load(os.path.join(self.root_path, self.data_path))
        self.data_x = data_raw.reshape(-1, 4*15*11*15) # 输入特征
        ...
        if self.scale:  # 假设 self.scale 是一个布尔变量，控制是否进行归一化
            train_data_x = data_raw.reshape(-1,4*15*11*15)[0:num_train]
            ...
            if self.set_type == 0:  # 训练集
                self.data_x = data_raw.reshape(-1,4*15*11*15)[0:num_train]
            elif self.set_type == 1:  # 验证集
                self.data_x = flag_x
            else:  # 测试集
                self.data_x = flag_x
    ...
    def __len__(self):
        return min(len(self.data_x), len(self.data_y)) - self.seq_len + 1
    ...
    def __getitem__(self, index):
        ...
        # 补齐 seq_y（防止越界或末尾不足）
        if seq_y.shape[0] < self.seq_len:
            pad_len = self.seq_len - seq_y.shape[0]
            pad = np.zeros((pad_len, seq_y.shape[1]))
