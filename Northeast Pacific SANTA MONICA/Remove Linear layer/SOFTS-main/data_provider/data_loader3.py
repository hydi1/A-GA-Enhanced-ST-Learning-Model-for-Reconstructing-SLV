...
class Dataset_Npy(Dataset):
    ...
    def __read_data__(self):
        ...
        else: #测试集data_loader.py
            self.data_x = self.scaler_x.transform(flag_x)
        # 归一化目标数据 y
        train_data_y = self.data_y[border1s[0]:border2s[0]]
        self.scaler_y = MinMaxScaler()
        train_data_y_fit = self.scaler_y.fit_transform(train_data_y)
        if self.set_type == 0: #训练集
            self.data_y=train_data_y_fit
        elif self.set_type == 1: #验证集
            self.data_y = self.scaler_y.transform(flag_y)
        else: #测试集  这里没有使用验证集
            self.data_y = self.scaler_y.transform(flag_y)
        print("self.scaler_y1",self.scaler_y)
    ...
    #     if is_target:
    #         print('self.scaler_y',self.scaler_y)
