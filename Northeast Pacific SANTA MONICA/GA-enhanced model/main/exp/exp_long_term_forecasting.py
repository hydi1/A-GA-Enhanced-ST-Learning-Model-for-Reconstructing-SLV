...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        # 支持 model_dict 中存放模块（包含 Model 类）或直接存放 Model 类/可调用构造器两种情况
    ...
    def test(self, setting, test=0):
        import pandas as pd  # ✅ 新增：用于写 Excel
        ...
        with torch.no_grad():
            ...
            r2_eff_norm = self.get_r2_eff(outputs_full, batch_y_full)

            # 反归一化去重叠后的数据
            outputs_full_unnorm = test_data.inverse_transform(outputs_full, is_target=True)
            batch_y_full_unnorm = test_data.inverse_transform(batch_y_full, is_target=True)

            # =========================================================
            # ✅ 新增：保存去重叠后的反归一化序列到 Excel
            # =========================================================
            # 保证是二维 (T, D)
            if outputs_full_unnorm.ndim == 1:
                outputs_full_unnorm = outputs_full_unnorm.reshape(-1, 1)
            if batch_y_full_unnorm.ndim == 1:
                batch_y_full_unnorm = batch_y_full_unnorm.reshape(-1, 1)

            T, D = outputs_full_unnorm.shape
            data_dict = {"time_idx": np.arange(T)}

            if D == 1:
                data_dict["pred"] = outputs_full_unnorm[:, 0]
                data_dict["true"] = batch_y_full_unnorm[:, 0]
            else:
                for d in range(D):
                    data_dict[f"pred_{d}"] = outputs_full_unnorm[:, d]
                for d in range(D):
                    data_dict[f"true_{d}"] = batch_y_full_unnorm[:, d]

            df = pd.DataFrame(data_dict)

            # 保存目录（与 txt 统一放一起）
            base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            save_dir = os.path.join(base_dir, 'test_result')
            os.makedirs(save_dir, exist_ok=True)

            save_path_excel = os.path.join(
                save_dir,
                f"{setting}_seed{self.args['model_id']}_lr{self.args['learning_rate']}_deoverlap_denorm.xlsx"
            )
            df.to_excel(save_path_excel, index=False)
            print(f"[Saved] De-overlapped de-normalized series to: {save_path_excel}")

            # =========================================================
            # 反归一化尺度（De-normalized）去重叠全序列指标
            # =========================================================
            rmse_unnorm = np.sqrt(np.mean((outputs_full_unnorm - batch_y_full_unnorm) ** 2))
            mae_unnorm = np.mean(np.abs(outputs_full_unnorm - batch_y_full_unnorm))

            # 打印结果
            print("\n" + "=" * 50)
            print(f"De-overlapped Normalized:   RMSE={rmse_norm:.4f}, MAE={mae_norm:.4f}, R2_eff={r2_eff_norm:.4f}")
            print(f"De-overlapped Unnormalized: RMSE={rmse_unnorm:.4f}, MAE={mae_unnorm:.4f}")
            print("=" * 50 + "\n")

            # 保存到 txt 文件
            save_path_txt = os.path.join(save_dir, 'test_results.txt')
            with open(save_path_txt, 'a') as f:
                f.write(f"seed={self.args['model_id']} | lr={self.args['learning_rate']}\n")
                f.write(f"[Normalized]    RMSE: {rmse_norm:.4f}, MAE: {mae_norm:.4f}, R2_eff: {r2_eff_norm:.4f}\n")
                f.write(f"[De-normalized] RMSE: {rmse_unnorm:.4f}, MAE: {mae_unnorm:.4f}\n\n")

        return {
            'rmse_norm': rmse_norm,
            'mae_norm': mae_norm,
            'r2_eff_norm': r2_eff_norm,
