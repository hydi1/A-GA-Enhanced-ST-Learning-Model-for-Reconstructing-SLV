...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        import pandas as pd  # ✅ 新增：保存 Excel
        ...
        with torch.no_grad():
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                outputs_cpu = outputs.detach().cpu().numpy().reshape(-1, 1)
                batch_y_cpu = batch_y.detach().cpu().numpy().reshape(-1, 1)
            ...
            # 你原代码里硬编码 full_length=33，这里保持不改结构
            full_length = 33
            ...
            # ===== 指标 1：归一化尺度去重叠后的 RMSE / MAE / R2_eff =====
            ...
            # =========================
            # ✅ 新增：保存“反归一化 + 去重叠”后的真实值/预测值到 Excel
            # =========================
            # 统一成二维 (T, D)
            if outputs_full_unnormalized.ndim == 1:
                outputs_full_unnormalized = outputs_full_unnormalized.reshape(-1, 1)
            if batch_y_full_unnormalized.ndim == 1:
                batch_y_full_unnormalized = batch_y_full_unnormalized.reshape(-1, 1)

            T, D = outputs_full_unnormalized.shape
            data_dict = {"time_idx": np.arange(T)}

            if D == 1:
                data_dict["true"] = batch_y_full_unnormalized[:, 0]
                data_dict["pred"] = outputs_full_unnormalized[:, 0]
            else:
                for d in range(D):
                    data_dict[f"true_{d}"] = batch_y_full_unnormalized[:, d]
                for d in range(D):
                    data_dict[f"pred_{d}"] = outputs_full_unnormalized[:, d]
            ...
            # 文件名包含 setting / seed / lr，避免覆盖
            save_path_excel = os.path.join(
                save_dir_excel,
                f"{setting}_seed{self.args['model_id']}_lr{self.args['learning_rate']}_deoverlap_denorm.xlsx"
            )
            df_denorm.to_excel(save_path_excel, index=False)
            print(f"[Saved] De-overlapped denormalized true/pred to: {save_path_excel}")
            # =========================
            ...
            if isinstance(setting, dict) and 'epoch' in setting:
                test_result_norm = (
                    ...
                    f"Deoverlap Normalized: rmse: {rmse_full_norm_avg}, mae: {mae_full_norm_avg}, r2_eff: {r2_full_norm_avg}"
                )
                test_result_denorm = (
                    f"Epoch: {setting['epoch']}, lr={self.args['learning_rate']}, "
                    f"Deoverlap Denormalized: rmse: {rmse_full_denorm_avg}, mae: {mae_full_denorm_avg}"
                )
            else:
                test_result_norm = (
            ...
            with open(save_path_txt, 'a') as f:
                ...
                f.write(f"Excel: {save_path_excel}\n")
                f.write('\n')
        ...
        return {
            ...
            'excel_path_denorm': save_path_excel
        }
