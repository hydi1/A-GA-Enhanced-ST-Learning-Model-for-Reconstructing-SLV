    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # ✅ 和 NEPO 对齐：decoder 输入为 0（并放到同 device 更稳）
                ...
                # ✅ 和 NEPO 对齐：保留 [B, L, D] 形状
                f_dim = -1 if self.args['features'] == 'MS' else 0
                outputs = outputs[:, :, f_dim:].detach().cpu().numpy()  # (B, L, 1) usually
                batch_y = batch_y[:, :, f_dim:].detach().cpu().numpy()  # (B, L, 1)
        ...
        # ✅ 去重叠拼接（NEPO 方式）
        # =========================
        total_samples = len(test_data)  # 窗口数
        print('total_samples',total_samples)
        seq_len = self.args['seq_len']  # 一般 12

        # --- full_length 的两种方式 ---
        # 方式1（推荐）：严格按滑窗理论
        full_length = total_samples + seq_len - 1

        # 方式2（如果你必须固定长度，比如你原来写死 33）
        # FULL_LENGTH_NEAO = 33
        # full_length = FULL_LENGTH_NEAO

        outputs_full = np.zeros((full_length, 1), dtype=np.float64)
        batch_y_full = np.zeros((full_length, 1), dtype=np.float64)
        counts = np.zeros(full_length, dtype=np.float64)

        sample_global_idx = 0
        for b_out, b_y in zip(all_outputs, all_batch_y):
            batch_size = b_out.shape[0]  # B
            # 注意：这里 seq_len 理论上应该等于 b_out.shape[1]
            # 但为了完全复刻 NEPO 逻辑，我们仍用 seq_len 来控制长度
        ...
        # ✅ 归一化尺度指标（NEPO 新增）
        # =========================
        rmse_norm = float(np.sqrt(np.mean((outputs_full - batch_y_full) ** 2)))
        mae_norm = float(np.mean(np.abs(outputs_full - batch_y_full)))
        
        # ✅ 新增：归一化尺度 + 去重叠全序列 的 R2_eff
        r2_eff_norm_full = self.get_r2_eff(outputs_full, batch_y_full) if hasattr(self, "get_r2_eff") else None
        ...
        rmse = float(np.sqrt(np.mean((outputs_final - targets_final) ** 2)))
        mae = float(np.mean(np.abs(outputs_final - targets_final)))

        # =========================
        # ✅ 保存去重叠反归一化序列到 Excel（NEPO 格式）
        # =========================
        ...
        save_dir = r"D:\sea level variability\code_neao\消融实验\GAconvgru -移除Series Embedding\SOFTS-main\test_result"
        ...
        if r2_eff_norm_full is None:
            print(f"[Normalized]    RMSE: {rmse_norm:.4f} | MAE: {mae_norm:.4f}")
        else:
        ...
        with open(save_path_txt, "a") as f:
            f.write(f"Setting: {setting} | seed={self.args['model_id']} | lr={self.args['learning_rate']}\n")
            if r2_eff_norm_full is None:
                f.write(f"[Normalized]    RMSE: {rmse_norm:.4f}, MAE: {mae_norm:.4f}\n")
            else:
