...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            all_outputs = []  # 保存所有预测结果（归一化尺度）
            all_batch_y = []  # 保存所有真实值（归一化尺度）

            # 计算基于 batch 的反归一化损失 + 收集用于去重叠的归一化序列
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # ⚠️ 这里你写死了 12，如果 seq_len 不是 12 请改为 self.args['seq_len']
                seq_len = self.args['seq_len']
                outputs_unnormalized_tensor = torch.from_numpy(outputs_unnormalized).float().reshape(-1, seq_len, 1)
                batch_y_unnormalized_tensor = torch.from_numpy(batch_y_unnormalized).float().reshape(-1, seq_len, 1)
                ...
                # =========================
                # 保存每个 batch 的归一化输出，用于后面去重叠（归一化尺度）
                # =========================
                all_outputs.append(outputs_cpu)  # [batch_size * seq_len, 1] (normalized)
                all_batch_y.append(batch_y_cpu)  # [batch_size * seq_len, 1] (normalized)
            ...
            # =========================
            # ✅ 新增：归一化尺度（去重叠后）RMSE / MAE 记录
            # =========================
            rmse_norm_val = float(np.sqrt(np.mean((outputs_full - batch_y_full) ** 2)))
            mae_norm_val = float(np.mean(np.abs(outputs_full - batch_y_full)))
            rmse_full_norm.update(rmse_norm_val, full_length)
            mae_full_norm.update(mae_norm_val, full_length)
            ...
            # =========================
            # 保存去重叠后的反归一化序列到 Excel（你原来的格式：2行×T列）
            # =========================
            ...
            # =========================
            # ✅ 新增：把“归一化RMSE/MAE”和“反归一化RMSE/MAE”也写进 Excel（另一个sheet）
            # =========================
            ...
            # 追加写入同一个Excel文件（新sheet），不覆盖原sheet
        ...
        with open(save_path_txt, 'a') as f:
            if isinstance(setting, dict) and 'epoch' in setting:
                test_result = (
                    ...
                    f"Batch-based(denorm): rmse: {rmse_batch.avg}, mae: {mae_batch.avg}, "
                    f"Full-seq(norm): rmse: {rmse_full_norm.avg}, mae: {mae_full_norm.avg}, "
                    f"Full-seq(denorm): rmse: {rmse_full.avg}, mae: {mae_full.avg}"
                )
            else:
                test_result = (
        ...
        return {
            ...
            'rmse_batch_denorm_avg': rmse_batch.avg,
            'mae_batch_denorm_avg': mae_batch.avg,
        }
