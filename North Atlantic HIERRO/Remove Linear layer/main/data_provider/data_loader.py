...
class Dataset_Npy(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.scale:  # 假设 self.scale 是一个布尔变量，控制是否进行归一化
            train_data_x = data_raw.reshape(-1,4*33*5*9)[0:num_train]
            ...
            if self.set_type == 0:  # 训练集
                self.data_x = data_raw.reshape(-1,4*33*5*9)[0:num_train]
            elif self.set_type == 1:  # 验证集
                self.data_x = flag_x
            else:  # 测试集
                self.data_x = flag_x
